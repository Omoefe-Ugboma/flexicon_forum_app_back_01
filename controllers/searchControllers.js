...
const Search = async (req, res) => {
  ...
  const skipValue = (page - 1) * resultLimit
  ...
  try {
    ...
    const postResults = await Post.find({
      $text: { $search: trimquery, $caseSensitive: false },
    })
      .skip(skipValue)
      .limit(resultLimit)
    ...
    const noMatches = Object.values(allResults).every(
    ...
    if (noMatches) {
      ...
      return res
        .status(404)
        .json({ message: 'No matches found for the given query' })
    }
